...
| テクニック名 | 概要（短く要点） | こういうシーンにおすすめ |
| --- | --- | --- |
| **Zero-shot** | 例示なしでタスクを実行させるプロンプト。指示だけで目的を伝える。 | ・明確な出力形式がある<br>・一般的な知識タスク<br>・回答に創造性を求めない時 |
| **Few-shot** | 入力例（Input→Outputのサンプル）を提示してフォーマットや意図を学習させる。 | ・形式が複雑な出力<br>・文体やトーンを模倣させたい<br>・誤解が起きやすいタスク |
| **Chain-of-Thought (CoT)** | 推論を「ステップに分解」して考えさせる。論理的タスクを強化。 | ・数学、論理、推論系タスク<br>・計算ミスが多いとき<br>・推論を丁寧にさせたい時 |
| **Zero-shot CoT** | 例を与えずに「Let's think step by step」などの短い指示だけでCoT推論モードへ誘導する。 | ・CoTを使いたいが例示を作るのが面倒<br>・短いプロンプトで推論精度を上げたい |
| **Self-Consistency** | CoTで複数回答を生成し、その中から最も一貫した答えを選ぶことで精度向上。 | ・推論にばらつきが出るタスク<br>・複雑な計算<br>・論理問題<br>・不安定な答えを安定化したい |
| **Generate Knowledge（GK）** | 回答前に「関連知識を生成させる」ことで推論を強化する。知識強化型CoT。 | ・専門知識を伴う推論<br>・未知領域に関する判断タスク<br>・曖昧な質問の精度向上 |
| **ReAct** | Reasoning（推論）＋ Acting（行動）を統合。推論しながらツール（検索等）を使う。 | ・RAG、外部検索が必要なタスク<br>・回答プロセスを透明化したい時<br>・外部情報依存タスク |
| **Directional Stimulus Prompting** | 曖昧なタスクに対し「方向性（Direction）」を与えることで回答を安定化する。 | ・抽象的なタスク(アイデア生成など)<br>・回答の"方向"をコントロールしたい<br>・創造性を保ちつつ軸を与えたい |
| **Multimodal CoT** | 画像＋テキストの「マルチモーダル」入力にCoTを適用。画像理解＋推論を可能にする。 | ・画像説明＋推論<br>・OCRの文脈理解<br>・画像に基づく判断タスク（例:医療、地図、図表） |
...
**ステップバイステップ**とは  
複雑な問題を、順番に段階を追って考えさせるためのプロンプト指示のこと。

以下のような構造で回答される傾向がある。  

| # | 段階 | 内容 |
|---------|------|------|
| 1 | 問題の整理 | 現状分析を行いながら、現在抱えている課題の整理を行う |
| 2 | 提案、選択肢の列挙 | 問題に合わせて、取りうる方法やアプローチを提示する |
| 3 | 比較・評価 | メリット・デメリットを整理して、最適解を導く |
| 4 | 結論 | 最終的な推奨案を提示する |
| 5 | 実行ステップ(内容に応じて) | 実際にどう進めるか、具体的な手順を提示 |

### こういうシーンにおすすめ(Zero-shot Chain-of-Thought)
- CoTを使いたいが例示を作るのが面倒
- 短いプロンプトで推論精度を上げたい
- 「うまく言語化できていない課題」「どう考えていいかわからない課題」をAIに考えさせたい
  - 「ステップバイステップで考えて」と指定すると、AIが論理的に推論してくれる
